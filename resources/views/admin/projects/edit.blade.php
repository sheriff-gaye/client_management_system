    ...
    <form action="{{route('project.update',$project->id)}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mt-2">
            <label class="block text-sm">
                ...
                <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input  @error('title') border-red-800 @enderror" placeholder="Title" name="title" value="{{$project->title}}" />
                @error('title')
        ...
        <div class="mt-2">
            <label class="block text-sm">
                ...
                <input type="date" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input  @error('deadline') border-red-800 @enderror" name="deadline" value="{{$project->deadline}}" />
                @error('deadline')
                <p class="text-red-700 text-xs">{{ $message }}</p>
                @enderror
            </label>
        ...
        <div class="mt-4">
            <label class="block mt-4 text-sm">
                ...
                <option value="{{$id}}" {{old('user_id', $project->user->id)==$id ? 'selected' : ''}}>{{$user}}</option>
        ...
        <div class="mt-4">
            <label class="block mt-4 text-sm">
                ...
                <select name="client_id" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                   ...
                   <option value="{{$id}}" {{old('client_id', $project->clients->id)==$id ? 'selected' : ''}}>{{$client}}</option>
                   @endforeach
        ...
        <div class="mt-4">
            <label class="block mt-4 text-sm">
                ...
                <option value="{{$status}}" {{old('ststaud', $project->status)==$status ? 'selected' : ''}}>{{ucfirst($status)}}</option>
                @endforeach    
        ...
        <div class="mt-4">
            <button class="bg-red-600 text-white font-medium rounded py-2 px-3" type="submit">Update</button>
