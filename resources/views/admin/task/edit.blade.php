...
<div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
    <form action="{{route('crm.task.update',$task->id)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="mt-2">
            <label class="block text-sm">
                <span class="text-gray-700 dark:text-gray-400">Project Tilte</span>
                <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input  @error('title') border-red-800 @enderror" placeholder="Title" name="title" value="{{$task->title}}" />
                @error('title')
        ...
        <div class="mt-2">
            <label class="block text-sm">
                ...
                <input type="date" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input  @error('deadline') border-red-800 @enderror" name="deadline" value="{{$task->deadline}}" />
                @error('deadline')
                <p class="text-red-700 text-xs">{{ $message }}</p>
                @enderror
            </label>
        ...
        <div class="mt-4">
            <label class="block mt-4 text-sm">
                ...
                <option value="{{$id}}" {{old('user_id', $task->user->id)==$id ? 'selected' : ''}}>{{$user}}</option>
        ...
        <div class="mt-4">
            <label class="block mt-4 text-sm">
                ...
                @foreach ($clients as $id=>$client )
                   <option value="{{$id}}" {{old('client_id', $task->client->id)==$id ? 'selected' : ''}}>{{$client}}</option>
                   @endforeach
        ...
        <div class="mt-4">
            <label class="block mt-4 text-sm">
                ...
                <select name="project_id" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                <option value="">--select</option>   
                @foreach ($projects as $id=>$project )
                   <option value="{{$id}}" {{old('project_id' , $task->project->id)==$id ? 'selected' : ''}}>{{$project}}</option>
                   @endforeach
